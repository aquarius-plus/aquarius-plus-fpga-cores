...
// Tile data
extern const unsigned int __tile_palette[32];
extern const unsigned int __tile_data[];
extern const int __tile_data_length[];
...
/*  -- ORIGINAL get_joystick() function --
...
static void init(void) {
    ...
    for (uint8_t i = 0; i < 32; i++) {
        IO_VPALSEL  = 32 + i;
        IO_VPALDATA = __tile_palette[i];
    }
    // Copy tile data into VRAM
    memcpy(vram_tiledata, __tile_data, __tile_data_length - __tile_data);
