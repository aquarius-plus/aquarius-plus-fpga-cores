#include "scr.h"

screen_t *scr_get_current(void) {
    switch (state.mode) {
        default:
        case MODE_CODE: return &scr_code;
        case MODE_SPRITE: return &scr_sprite;
        case MODE_MAP: return &scr_map;
        case MODE_SFX: return &scr_sfx;
        case MODE_MUSIC: return &scr_music;
    }
}

void scr_common(unsigned bg_col) {
    fill_rect(&(rect_t){0, 0, 199, 6}, 8);
    fill_rect(&(rect_t){0, 7, 199, 152}, bg_col);
    fill_rect(&(rect_t){0, 159 - 6, 199, 159}, 2);

    // Mode icons
    {
        int x = 200 - 5 * 8 - 1;

        rect_t r = {x, 0, x + 6, 6};

        for (int i = 0; i <= MODE_MUSIC; i++) {
            // fill_rect(&r, 0);
            if (mouse_lclick(&r))
                state.mode = i;

            draw_icon(r.x0 + 1, 1, i, state.mode == i ? 7 : 2);

            r.x0 += 8;
            r.x1 += 8;
        }

        // draw_icon(x, 1, 0, state.mode == MODE_CODE ? 7 : 2);
        // x += 8;
        // draw_icon(x, 1, 1, state.mode == MODE_SPRITE ? 7 : 2);
        // x += 8;
        // draw_icon(x, 1, 2, state.mode == MODE_MAP ? 7 : 2);
        // x += 8;
        // draw_icon(x, 1, 3, state.mode == MODE_SFX ? 7 : 2);
        // x += 8;
        // draw_icon(x, 1, 4, state.mode == MODE_MUSIC ? 7 : 2);
    }

    const char *title = "";
    switch (state.mode) {
        case MODE_CODE: title = "Code editor"; break;
        case MODE_SPRITE: title = "Sprite editor"; break;
        case MODE_MAP: title = "Map editor"; break;
        case MODE_SFX: title = "Sound effects editor"; break;
        case MODE_MUSIC: title = "Music editor"; break;
    }
    draw_text(title, 1, 1, 15, true);
}

void scr_key(uint16_t key) {
    if (key == CH_F1) {
        state.mode = MODE_CODE;
    } else if (key == CH_F2) {
        state.mode = MODE_SPRITE;
    } else if (key == CH_F3) {
        state.mode = MODE_MAP;
    } else if (key == CH_F4) {
        state.mode = MODE_SFX;
    } else if (key == CH_F5) {
        state.mode = MODE_MUSIC;
    } else {

        screen_t *scr = scr_get_current();
        if (scr->on_key)
            scr->on_key(key);
    }
}
